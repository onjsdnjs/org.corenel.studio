...
public abstract class AbstractMessageServiceHelper extends GenericServiceHelper implements MessageServiceHelper{
	...
	public AbstractMessageServiceHelper(Context<String, Object> context) {
		...
		super(context);
		messageService = getServiceContext().getBean(ApplicationConstants.MESSAGE_SERVICE, MessageService.class);
		sendMessageVO = getServiceContext().getBean(ApplicationConstants.EXCEL_MODEL_VO, SendMessageVO.class);
	}
	
	@Override
	public void executeService() throws Exception{
		getServiceContext().putBean(ApplicationConstants.MESSAGE_RESULT_VO, getMessageService().sendMessage(getServiceContext().getBean(ApplicationConstants.EXCEL_MODEL_VO, SendMessageVO.class)));
	}
